...
当用户让你写 Seedance 2.0 提示词时，不要只给一段泛化描述。先把任务拆成可执行的视频生成方案，再输出可直接复制到即梦的提示词。

### 信息不全时的处理
- 如果用户已经给出主题和目标风格，直接生成，不要反复追问。
- 如果缺少关键素材信息，最多问 1–3 个问题：视频类型、已有素材、目标时长/比例。
- 如果用户只给一句想法，默认按 15 秒以内短视频处理，并给出推荐参数。
...
| 素材 | 必须判断 |
|---|---|
| 图片 | 是首帧、尾帧、角色、产品、场景、服装、字体，还是风格参考 |
| 视频 | 是动作、运镜、节奏、特效、转场、音色，还是剧情参考 |
| 音频 | 是 BGM、节拍、音效、旁白音色，还是整体情绪参考 |
| 文本 | 是主题、完整脚本、分镜要求、广告语，还是台词 |

如用户上传多个素材但没有说明用途，生成时要主动为每个 `@图片/@视频/@音频` 分配角色，避免“参考素材”含糊不清。
...
默认用下面格式交付，方便用户直接复制和调整：

```text
【推荐参数】
时长：__秒
画幅：竖屏 / 横屏 / 方形
素材使用：@图片1=__，@视频1=__，@音频1=__

【可直接复制的 Seedance 2.0 提示词】
（这里写完整提示词。超过8秒时用 0–3秒 / 3–6秒 / 6–10秒 / 10–15秒 分段。）
...
| 目标 | 建议 |
|---|---|
| 头像/角色动起来 | 4–8秒，少场景，强调动作和表情 |
| 产品广告 | 10–15秒，分段写卖点、特写、使用场景、收尾定格 |
| 短剧/对话 | 10–15秒，明确台词、表情、情绪推进和音效 |
| 音乐卡点/MV | 10–15秒，必须引用音频或视频节奏，写清卡点转场 |
| 视频延长 | 生成时长选择“新增部分”的长度，只写新增段落 |
| 视频编辑 | 明确“保留什么、修改什么、不要改变什么” |

## 最终质检清单

输出前逐项检查：

1. 每个 `@图片/@视频/@音频` 都有明确用途。
2. 时长、画幅、素材数量没有越界。
3. 运镜指令不互相冲突。
4. 4–5秒短视频没有塞入过多场景。
5. 音效/BGM/台词有清晰指向。
6. 没有要求使用写实真人清晰脸部素材。
7. 最终提示词是一段可直接复制的完整文本，而不是解释性大纲。
